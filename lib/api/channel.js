...
export default class Channel {
  ...
  _handleMessage (message) {
    // parent window (contentful web app) tells iframe handlers for which signal should be called
    if (message.method) {
      const {method, params} = message
      // signal you want to dispatch data for (should be singular)
      const handlers = this._messageHandlers[method]
      if (handlers) {
        // call all attached handlers for the signal corresponding to `message.method`
        handlers.dispatch(...params)
      }
    } else {
      // response to some rpc
      // the id of the message used to request rpc in contentful web app is store in `id`
      // The responseHandler[id] returns {resolve, reject}
